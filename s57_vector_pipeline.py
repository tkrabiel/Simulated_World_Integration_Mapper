...
def generate_buoy_ini(db_dir, ini_path):
    ...
    # SAFELY FILTER BEFORE CONCATENATING
    valid_gdfs = [gdf for gdf in gdfs if not gdf.empty]
    buoys = pd.concat(valid_gdfs, ignore_index=True) if valid_gdfs else gpd.GeoDataFrame()
    
    if buoys.empty: 
        print("[-] No buoys found in database. Skipping buoy.ini generation.")
        return
    ...
    if buoys.empty: 
        print("[-] No active buoys found in database. Skipping buoy.ini generation.")
        return
    ...
    with open(os.path.join(ini_path, "buoy.ini"), "w") as f:
        ...
        for count, row in enumerate(buoys.itertuples(), start=1):
...
def generate_bcn_ini(db_dir, ini_path):
    ...
    # SAFELY FILTER BEFORE CONCATENATING
    valid_gdfs = [gdf for gdf in gdfs if not gdf.empty]
    bcns = pd.concat(valid_gdfs, ignore_index=True) if valid_gdfs else gpd.GeoDataFrame()

    if bcns.empty: 
        print("[-] No beacons found in database. Skipping landobject.ini generation.")
        return
    ...
    if bcns.empty: 
        print("[-] No active beacons found in database. Skipping landobject.ini generation.")
        return

    os.makedirs(ini_path, exist_ok=True)
    total_count = len(bcns)
    
    with open(os.path.join(ini_path, "landobject.ini"), "w") as f:
        f.write(f"Number={total_count}\n")
        
        for count, row in enumerate(bcns.itertuples(), start=1):
...
def generate_light_ini(db_dir, ini_path, height_eye=15):
    ...
    if lights.empty: 
        print("[-] No lights found in database. Skipping light.ini generation.")
        return
    ...
    if lights.empty: 
        print("[-] No active lights found in database. Skipping light.ini generation.")
        return
    ...
    with open(os.path.join(ini_path, "light.ini"), "w") as f:
        ...
        for count, row in enumerate(lights.itertuples(), start=1):
            ...
            f.write(f'Long({count})={row.geometry.x}\nLat({count})={row.geometry.y}\nHeight({count})={height}\n')
