...
# 3\. \*\*Scrape Features:\*\* Toggle the S-57 layers you wish to extract (Lights, Buoys, Beacons, Land, Dredge, M\_COVR).
...
# | `main\_gui.py` | My main UI app. Handles downloading, multi-chart data scraping, spatial deduplication, and master GeoParquet compression. |

# | `s57\_vector\_pipeline.py` | My high-efficiency asset generator. Streams Parquet layers via fast tuples to write simulator files in milliseconds. |

# | `s57\_raster\_pipeline.py` | \*(I am still fixing this)\* Merges topography and bathymetry data using S-57 shape masks to export accurate `terrain.ini` profiles. |

# | `cloud\_dem\_pipeline.py` | \*(I am still fixing this)\* Automatically pulls chart boundaries (`M\_COVR`), queries cloud STAC catalogs, and streams clipped DEM rasters into memory. |

# | `s57\_dictionaries.py` | My lookup engine. Maps raw S-57 codes to simulator definitions with my built-in protection against NumPy truth-value crashes and float artifacts. |
